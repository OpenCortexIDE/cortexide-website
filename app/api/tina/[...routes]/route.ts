...
async function getHandler() {
  ...
  if (!databaseClient) {
    try {
      ...
      const dbPath = join(process.cwd(), 'tina', '__generated__', 'databaseClient.js')
      databaseClient = eval('require')(dbPath).default
    } catch (e) {
      // If generated client doesn't exist, try database.ts as fallback
      try {
        const dbPath = join(process.cwd(), 'tina', 'database.ts')
        // For TypeScript files, we need to import them differently
        // But in production, they should be compiled to JS
