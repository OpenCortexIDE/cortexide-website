...
async function getHandler() {
  ...
  if (!databaseClient) {
    try {
      ...
      // Try multiple approaches to find the database client
      const possiblePaths = [
      ...
      // Log available files in __generated__ directory for debugging
      try {
        const fs = require('fs')
        const path = require('path')
        const genDir = path.join(process.cwd(), 'tina', '__generated__')
        if (fs.existsSync(genDir)) {
          console.error('Files in __generated__:', fs.readdirSync(genDir))
        } else {
          console.error('__generated__ directory does not exist')
        }
      } catch (debugErr) {
        console.error('Could not read __generated__ directory:', debugErr)
      }
      return null
    }
  }
